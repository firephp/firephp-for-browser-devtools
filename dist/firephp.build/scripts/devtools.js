...
},{"codeblock/codeblock.rt0":1,"pinf-loader-js":4}],3:[function(require,module,exports){
"use strict";
...
},{"./jsonrep":2}],4:[function(require,module,exports){
"use strict";
...
(function (global) {
	...
	if (!global || (typeof global === "undefined" ? "undefined" : _typeof(global)) !== "object") {
	...
	function normalizeSandboxArguments(implementation) {
		return function (programIdentifier, options, loadedCallback, errorCallback) {
			/*DEBUG*/if (typeof options === "function" && (typeof loadedCallback === "undefined" ? "undefined" : _typeof(loadedCallback)) === "object") {
				...
				if (/^[^\/]*\/package.json$/.test(key)) {
					...
					if (loadedBundles[0][1][key][0].mappings) {
						for (var alias in loadedBundles[0][1][key][0].mappings) {
							...
							pending += 1;
							loadInBrowser(rebaseUri(loadedBundles[0][1][key][0].mappings[alias]), function () {
								pending -= 1;
								finalize();
							});
						}
	...
	var Loader = function Loader(bundleGlobal) {
		...
		var Require = function Require(bundle) {
			...
			var bundleHandler = function bundleHandler(uid, callback) {
				...
				req.memoize = function (moduleIdentifier, moduleInitializer, moduleMeta) {
					...
					moduleMeta && moduleMeta.variation ? ":" + moduleMeta.variation : "")] = [moduleInitializer, moduleMeta || {}];
				};
	...
	if (typeof global.addEventListener === "function") {
		global.addEventListener("message", function (event) {
			var m = null;
			if (typeof event.data === "string" && (m = event.data.match(/^notify:\/\/pinf-loader-js\/sandbox\/load\?uri=(.+)$/)) && (m = decodeURIComponent(m[1])) &&
